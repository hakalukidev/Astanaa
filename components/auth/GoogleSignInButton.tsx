...
/**
 * Plain link (not fetch) so the browser follows the redirect chain to Google
 * and back; the callback sets the session cookie and lands on the homepage.
 */
export default function GoogleSignInButton({ label }: GoogleSignInButtonProps) {
  return (
    <a
      href="/api/auth/google"
      className="flex h-10 w-full items-center justify-center gap-3 rounded-md border border-gray-300 bg-white px-4 text-sm font-medium text-gray-700 transition hover:bg-gray-50"
    >
