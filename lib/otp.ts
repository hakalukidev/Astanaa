...
/** Normalizes a raw email/phone into the identifier we key OTP rows and lookups by. */
...
export async function sendOtp(input: {
  ...
  const id = docId(purpose, identifier);

  const existing = await db.otp.findUnique({ where: { id } });
  ...
  try {
    ...
    await db.otp.delete({ where: { id } }).catch(() => undefined);
...
export async function verifyOtp(input: {
  ...
  const id = docId(purpose, identifier);
  const data = await db.otp.findUnique({ where: { id } });
  ...
  if (data.codeHash !== hashCode(identifier, purpose, code)) {
    await db.otp.update({ where: { id }, data: { attempts: data.attempts + 1 } });
